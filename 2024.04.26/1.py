...
def pick_resistors(value):
    # словарь рядов номиналов определен как nominals
    if not 100 <= value <= 999:
        return None

    def find_closest(series):
        return tuple(sorted(filter(lambda x: abs(x - value) == min(map(lambda y: abs(y - value), series)), series)))

    return {series_name: find_closest(series) for series_name, series in nominals.items()}
